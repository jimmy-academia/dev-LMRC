...
# New prompts for the recursive agent
RECURSIVE_SYSTEM_PROMPT = """You are a helpful search assistant that navigates hierarchical data.
You systematically explore categories, create subcategories, and find relevant products.
You use a recursive approach to explore the hierarchy from top to bottom.

Your goal is to:
1. Understand the user's query
2. Systematically explore relevant category paths
3. Create meaningful subcategories when appropriate
4. Find the most relevant items matching the query
5. Return a well-organized set of results
"""

ANALYZE_NODE_PROMPT = """
You are now examining a node in the category hierarchy.

Current path: {path}
Total items at this node: {total_items}
Subcategories: {subcategories}
Items directly at this node (not in subcategories): {direct_items}
Depth level: {level} (max allowed: {max_depth})

Based on the query: "{query}"

Analyze this node and decide:
1. Should we explore existing subcategories? If so, which ones?
2. Should we create new subcategories? If so, what would they be?
3. Should we search for items at this node? If so, what keywords?
4. Is this node irrelevant to the query and we should backtrack?

Return your analysis as valid JSON in the format:
{
  "reasoning": "<your analysis>",
  "decision": {
    "action": "<explore|create|search|backtrack>",
    "params": {
...
class RecursiveAgent:
    ...
    def search(self, query, gt, gt_cat, max_steps=5):
        ...
        # After recursive exploration, refine the final item set if needed
        if len(self.found_items) > 20:
            # If we have too many items, filter to the most relevant ones
            sorted_items = sorted(
                [(item_id, score) for item_id, score in self.item_relevance_scores.items()],
                key=lambda x: x[1],
                reverse=True
            )
            self.found_items = set([item_id for item_id, _ in sorted_items[:20]])
        
        # Prepare final results
        return self._prepare_final_results()
    
    def _recursive_search(self, path, level):
        """
        ...
        Returns:
            list: Relevant items found in this subtree
        """
        # Check if we've reached step limit or maximum depth
        if self.step_count >= self.max_steps or level >= self.max_depth:
            return []
